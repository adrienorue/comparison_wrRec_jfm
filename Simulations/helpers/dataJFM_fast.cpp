    ...
    double scaleDeath)
{
  ...
  // Disallow AH + log-logistic (as requested)
  ...
  for (int i = 0; i < nSubjects; ++i)
  {
    ...
    if (effect_type == PH)
    {
      const double accelD = std::pow(W, alpha) * std::exp(etaD);
      if (baseline_type == EXPONENTIAL)
      {
        Tdeath = sample_PH_exp(lambda0, accelD);
      }
      else
      { // LOGLOGISTIC
        Tdeath = sample_PH_llogis(scaleDeath, shapeDeath, accelD);
      }
    }
    else
    { // AH + EXPONENTIAL
      const double addD = alpha * W + etaD;
      Tdeath = sample_AH_exp(lambda0, addD);
    }
    ...
    while (true)
    {
      ...
      if (effect_type == PH)
      {
        const double accelR = W * std::exp(etaR);
        if (baseline_type == EXPONENTIAL)
        {
          gap = sample_PH_exp(r0, accelR);
        }
        else
        { // LOGLOGISTIC
          gap = sample_PH_llogis(scaleRec, shapeRec, accelR);
        }
      }
      else
      { // AH + EXPONENTIAL
        const double addR = W + etaR;
        gap = sample_AH_exp(r0, addR);
      }
